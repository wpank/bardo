...
//!
//! Each step has a system prompt and a function that builds the user message
//! from input file contents. Prompts are language-agnostic where possible
//! (detect language from project files, not hardcoded).
...
mod tests {
    ...
    #[test]
    fn brief_user_includes_plan() {
        let msg = brief_user("my plan content", None);
        assert!(msg.contains("my plan content"));
    }

    #[test]
    fn brief_user_includes_decomposition() {
        let msg = brief_user("plan", Some("decomp"));
        assert!(msg.contains("decomp"));
    }

    #[test]
    fn tasks_user_includes_plan() {
        let msg = tasks_user("plan content");
        assert!(msg.contains("plan content"));
    }
}
