use ratatui::layout::Rect;
use ratatui::style::Style;
use ratatui::text::{Line, Span};
use ratatui::widgets::Paragraph;
use ratatui::Frame;

use crate::state::{RunPlanStatus, RunState};
use crate::tui::atmosphere::Atmosphere;
use crate::tui::color;
use crate::tui::theme::Theme;

/// Render a TimelineRibbon showing wave execution as proportional segments.
/// Each wave gets a segment proportional to its plan count.
pub fn render(f: &mut Frame, area: Rect, state: &RunState, atmosphere: &Atmosphere) {
    if state.execution_waves.is_empty() {
        return;
    }

    let total_plans: usize = state.execution_waves.iter().map(|(_, p)| p.len()).sum();
    if total_plans == 0 {
        return;
    }

    let width = area.width as usize;
    if width < 10 {
        return;
    }

    let gradient = color::ocean_gradient();
    let mut spans: Vec<Span> = Vec::new();

    for (idx, (wave_num, wave_plans)) in state.execution_waves.iter().enumerate() {
        let wave_width =
            (wave_plans.len() as f64 / total_plans as f64 * width as f64).ceil() as usize;
        let wave_width = wave_width.max(3); // minimum 3 chars per wave

        let wave_done = wave_plans
            .iter()
            .filter(|base| {
                state.plans.iter().any(|p| {
                    &p.base == *base
                        && matches!(
                            p.status,
                            RunPlanStatus::Completed | RunPlanStatus::CompletedPrior
                        )
                })
            })
            .count();
        let wave_total = wave_plans.len();
        let fraction = if wave_total > 0 {
            wave_done as f64 / wave_total as f64
        } else {
            0.0
        };

        let is_current = idx == state.current_wave;
        let bar_color = if fraction >= 1.0 {
            Theme::SAGE
        } else if is_current {
            gradient.sample_color(0.7)
        } else {
            Theme::TEXT_GHOST
        };

        let filled = (fraction * wave_width as f64) as usize;

        // Wave label
        let label = format!("W{}", wave_num);
        let label_len = label.len();

        if wave_width > label_len + 1 {
            spans.push(Span::styled(
                format!("{label} "),
                Style::default().fg(if is_current {
                    Theme::BONE
                } else {
                    Theme::FG_DIM
                }),
            ));
            let bar_w = wave_width.saturating_sub(label_len + 1);
            let filled_bar = filled.min(bar_w);
            let empty_bar = bar_w.saturating_sub(filled_bar);
            if is_current && filled_bar > 0 {
                // Per-cell ocean gradient with animated offset
                for j in 0..filled_bar {
                    let t = (j as f64 / bar_w.max(1) as f64 + atmosphere.elapsed() * 0.1) % 1.0;
                    let c = gradient.sample_color(t);
                    spans.push(Span::styled("━", Style::default().fg(c)));
                }
            } else {
                spans.push(Span::styled(
                    "━".repeat(filled_bar),
                    Style::default().fg(bar_color),
                ));
            }
            spans.push(Span::styled(
                "░".repeat(empty_bar),
                Style::default().fg(Theme::TEXT_GHOST),
            ));
        } else {
            let empty = wave_width.saturating_sub(filled);
            if is_current && filled > 0 {
                for j in 0..filled {
                    let t =
                        (j as f64 / wave_width.max(1) as f64 + atmosphere.elapsed() * 0.1) % 1.0;
                    let c = gradient.sample_color(t);
                    spans.push(Span::styled("━", Style::default().fg(c)));
                }
            } else {
                spans.push(Span::styled(
                    "━".repeat(filled),
                    Style::default().fg(bar_color),
                ));
            }
            spans.push(Span::styled(
                "░".repeat(empty),
                Style::default().fg(Theme::TEXT_GHOST),
            ));
        }
    }

    let line = Line::from(spans);
    f.render_widget(Paragraph::new(line), area);
}
