...
pub struct PaymentRequired {
    /// Amount in USDC base units (6 decimals: 1_000_000 = $1.00).
    pub amount: U256,
    /// Token contract address (USDC on Base).
    pub asset: Address,
    /// Chain ID (8453 = Base).
    pub chain_id: u64,
    /// Recipient address (gateway's wallet).
...
pub struct CostBreakdown {
    /// Estimated provider cost in USDC base units.
    pub provider_cost: U256,
    /// Spread amount in USDC base units.
    pub spread: U256,
...
pub struct PaymentReceipt {
    ...
    /// Amount actually charged in USDC base units.
...
/// Attached to request extensions by the MPP middleware after credential validation.
#[derive(Debug, Clone)]
...
/// Auth mode tag set as a request extension.
