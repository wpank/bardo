...
> **Reader orientation:** This document specifies the inference parameter policies for every Golem (a mortal autonomous DeFi agent managed by the Bardo runtime) subsystem. It belongs to the Bardo Inference layer and defines temperature, sampling strategy, reasoning effort, and response format per subsystem and cognitive state. The key concept is that different subsystems need different inference parameters: risk assessment demands near-deterministic output (temperature 0.1), while dream cycles need creative recombination (temperature 0.8-0.9). Note: this document overlaps substantially with `15-inference-profiles.md`. For term definitions, see `prd2/shared/glossary.md`.
...
| **daimon** | 0.4 | top_p=0.9 | Low | `DaimonAppraisal` schema | ✗ | Emotional appraisal needs consistency but not rigidity. PAD vector extraction via structured output. Privacy preferred (Venice). |
...
| **hypnagogic_dali** | 1.2–1.5 | min_p=0.08 | None | None (free text) | ✗ | Peak creative range. Dalí interrupt: 50-100 token partials at max temperature. Highest temp in the entire system. |
...
Each Dalí cycle within hypnagogic onset follows a cosine schedule:
...
```rust
/// Cosine temperature annealing for Dalí cycles.
...
| --- | --- | --- | --- |
| 1 | System prompt (identity, archetype) | ✓ | Never (within a lifecycle) |
| 2 | STRATEGY.md (owner-authored) | ✓ | Rarely (owner edits) |
| 3 | PLAYBOOK.md (evolved heuristics) | ✓ | Every 50 ticks (Curator cycle) |
| 4 | Tool definitions (pruned) | ✓ | Per-tick (dynamic pruning) |
| 5 | Retrieved Grimoire entries | ✗ | Per-tick |
| 6 | Current market context | ✗ | Per-tick |
| 7 | User message / query | ✗ | Per-tick |
...
| --- | --- | --- | --- | --- | --- |
| Venice (Claude) | 90% | 25% | ~4,000 | 5 min | ✓ (gateway adds markers) |
| Venice (other) | 50-90% | None | ~1,024 | 5 min | ✓ (auto by prefix) |
| Anthropic (Direct) | 90% | 25% | ~4,000 | 5 min | ✓ (gateway adds markers) |
| OpenAI (Direct) | 90% | None | 1,024 | 5-10 min | ✓ (auto by prefix) |
| Bankr | Passthrough (depends on underlying) | Passthrough | Passthrough | Passthrough | ✓ |
| BlockRun | Passthrough | Passthrough | Passthrough | Passthrough | ✓ |
...
| Topic | Document |
| --- | --- |
| Model routing and provider resolution | [01a-routing.md](01a-routing.md) |
| Structured output schemas | [01-structured-outputs.md](01-structured-outputs.md) (companion doc) |
| Context engineering pipeline | [04-context-engineering.md](04-context-engineering.md) |
| Venice provider specification | [12-providers.md](12-providers.md) §Venice |
| Hypnagogic temperature scheduling | [../06-hypnagogia/02-architecture.md](../06-hypnagogia/02-architecture.md) |
| Dream cycle phases | [../05-dreams/01-architecture.md](../05-dreams/01-architecture.md) |
| Mortality pressure on inference | [01a-routing.md](01a-routing.md) §Mortality |
